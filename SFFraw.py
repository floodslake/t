...
if use_ffmpeg == 'true':
  ...
  download_command = 'streamlink.AppImage --stream-timeout 300 --stream-segment-attempts 300 --hls-playlist-reload-attempts 300 --hls-live-restart --retry-open 300 --retry-streams 30 --retry-max 600 --stream-segment-threads 8 --twitch-disable-hosting --twitch-disable-ads --twitch-disable-reruns --progress=force %s %s -o "DL/%s.ts" "%s" best' % (http_header, s_referer, output, m3u8_url)
